...
public class UserDAOImpl extends AbstractMySQLDAO implements UserDAO {
	...
	@Override
	public int delete(Long idDaRimouovere) throws Exception {
		// prima di tutto cerchiamo di capire se possiamo effettuare le operazioni
		if (isNotActive())
			throw new Exception("Connessione non attiva. Impossibile effettuare operazioni DAO.");

		if (idDaRimouovere == null || idDaRimouovere < 1)
			throw new Exception("Valore di input non ammesso.");

		int result = 0;
		try (PreparedStatement ps = connection.prepareStatement("DELETE FROM user WHERE ID=?")) {
			ps.setLong(1, idDaRimouovere);
			result = ps.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	public List<User> cercaTuttiQuelliCheUsernameIniziaCon(String iniziale) throws Exception {
		...
		if (iniziale == null || iniziale.length() < 1)
			throw new Exception("Valore di input non ammesso.");
...
//			ps.setDate(1, dataConfronto);
	...
	public List<User> cercaPerCognomeENomeCheInziaCon(String cognomeInput, String inzialeNomeInput) throws Exception {
		...
		if (cognomeInput == null || cognomeInput.length() < 1 || inzialeNomeInput == null || inzialeNomeInput.length() < 1)
			throw new Exception("Valori di input non ammesso.");
